...
export function HomePage() {
  const { data: messages = [], isLoading } = useMessages("ayush");
  ...
  return (
    <div className="flex justify-center items-center flex-col min-h-screen">
      ...
      <div className="w-full max-w-lg">
        <h2 className="w-full max-w-lg text-neutral-700">Unanswered Messages</h2>
        {messages.map((m) => (
          <MessageItem key={m.id} content={m.content} id={m.id} reply={m.reply} />
        ))}
      </div>
    </div>
  );
